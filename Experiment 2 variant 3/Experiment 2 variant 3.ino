//This code was used the second set of experiments and is able to produce the third pattern described in the report. It uses a synthetic heart rate signal rather than the pulse sensor
...
void loop() {
  ...
  //The below if statements allow the motor to pulse at specific timed intervals in the experiment time frame. This pattern (3) is explained and detailed in the report (Wearable Robotics for Augmenting Communication)

  if(startTime >= 00000 && startTime < 16500){
    currentBPM = 110;
  }
  if(startTime >= 16500 && startTime < 30000){
    currentBPM = 130;
  }
  if(startTime >= 30000){
    currentBPM = 60;
