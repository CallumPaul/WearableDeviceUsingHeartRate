//This code was used the second set of experiments and is able to produce the second pattern described in the report. It uses a synthetic heart rate signal rather than the pulse sensor
...
void loop() {
  ...
  //The below if statements allow the motor to pulse at specific timed intervals in the experiment time frame. This pattern (2) is explained and detailed in the report (Wearable Robotics for Augmenting Communication)

  if(startTime >= 00000 && startTime < 3750){
    currentBPM = 80;
  }
  if(startTime >= 3750 && startTime < 6500){
    currentBPM = 110;
  }  
  if(startTime >= 6500 && startTime < 7750){
    currentBPM = 190;
  }
  if(startTime >= 7750 && startTime < 11500){
    currentBPM = 80;
  }  
  if(startTime >= 11500 && startTime < 12750){
    currentBPM = 190;
  } 
  if(startTime >= 12750 && startTime < 15000){
    currentBPM = 130;
  }  
  if(startTime >= 15000 && startTime < 18750){
    currentBPM = 80;
  }
  if(startTime >= 18750 && startTime < 20000){
    currentBPM = 190;
  }  
  if(startTime >= 20000 && startTime < 22250){
    currentBPM = 130;
  }
  if(startTime >= 22250 && startTime < 26000){
    currentBPM = 80;
  }  
  if(startTime >= 26000 && startTime < 28750){
    currentBPM = 110;
  } 
  if(startTime >= 28750 && startTime < 30000){
    currentBPM = 190;
  }    
  if(startTime >= 30000){
    currentBPM = 70;
